...
function sumOfCodes(str) {
  return str ? [...str].reduce((acc, i) => acc + i.charCodeAt(0), 0) : 0;
}
...
function countVowels(str) {
  ...
  return [...str].filter((i) => vowels.includes(i)).length;
}
...
function isPalindrome(str) {
  const i = str.toLowerCase().replace(/[^a-z0-9]/g, '');
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (i) => {
    const startMove = i < 'a' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    return String.fromCharCode(
      startMove + ((i.charCodeAt(0) - startMove + 13) % 26)
    );
  });
}
